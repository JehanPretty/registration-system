...
def get_master_component():
  ...
  const getAutoColor = (customColor, lightVariant, darkVariant) => {
    ...
    if (customColor && customColor !== "" && customColor !== "#ffffff" && customColor !== "white") return customColor;
    ...
    return (
      <View style={{ flexDirection: compact ? 'row' : 'column', alignItems: 'center', gap: compact ? 12 * s : 8 * s }}>
        ...
        <View style={{ alignItems: compact ? 'flex-start' : 'center' }}>
          {headerText ? <Text style={{ fontSize: hSize, fontWeight: '900', color: hColor, letterSpacing: 1.5 * s, textTransform: 'uppercase', marginBottom: 2 * s }}>{headerText}</Text> : null}
          <Text style={{ fontSize: iSize, fontWeight: '900', color: iColor, letterSpacing: 0.5 * s, textAlign: compact ? 'left' : 'center' }}>{instName}</Text>
          {instSub ? <Text style={{ fontSize: sSize, fontWeight: '700', color: sColor, letterSpacing: 1 * s, marginTop: 2 * s, textAlign: compact ? 'left' : 'center', textTransform: 'uppercase' }}>{instSub}</Text> : null}
        </View>
      </View>
    );
  };
            ...
            <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 24 * s, overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9' }}>
              <View style={{ height: 200 * s, backgroundColor: primary, position: 'relative', overflow: 'hidden', paddingTop: 24 * s }}>
                {template.custom_front_bg_url ? (
                  <View style={{ position: 'absolute', inset: 0 }}>
                    ...
                    <LinearGradient colors={['transparent', 'white']} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 100 * s }} />
                  </View>
                ) : (
              ...
              {template.show_avatar && (
                <View style={{ alignItems: 'center', marginTop: -65 * s, zIndex: 20 }}>
                  <View style={{ width: 130 * s, height: 130 * s, borderRadius: 65 * s, borderWidth: 6 * s, borderColor: 'white', overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 20 * s, elevation: 10, backgroundColor: 'white' }}>
              ...
              <View style={{ paddingHorizontal: 32 * s, paddingBottom: 32 * s, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                {template.show_qr && <View style={{ padding: 10 * s, backgroundColor: '#f8fafc', borderRadius: 16 * s }}><QRCode value={qrPayload} size={60 * s} color={primary} /></View>}
                {template.show_id_number && (
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={{ fontSize: 8 * s, fontWeight: '900', color: '#cbd5e1', letterSpacing: 3 * s, textTransform: 'uppercase' }}>ID Number</Text>
          ...
          return (
            <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 24 * s, overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9', flexDirection: 'row' }}>
              ...
              <View style={{ flex: 1, padding: 32 * s, position: 'relative', overflow: 'hidden' }}>
                ...
                {template.show_avatar && (
                  <View style={{ width: 140 * s, height: 140 * s, borderRadius: 16 * s, overflow: 'hidden', borderWidth: 4 * s, borderColor: '#f8fafc', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10 * s, elevation: 5, backgroundColor: 'white', marginTop: 48 * s }}>
                    <Image source={{ uri: avatar, headers: { 'Bypass-Tunnel-Reminder': 'true' } }} style={{ width: '100%', height: '100%' }} contentFit="cover" />
                  </View>
                )}
                <View style={{ flex: 1, paddingTop: template.show_avatar ? 12 * s : 112 * s, position: 'relative', zIndex: 10 }}>
                  <Text style={{ fontSize: (template.name_font_size || 24) * s, fontWeight: '900', color: template.name_color || primary }}>{displayName}</Text>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto' }}>
                    {template.show_id_number && (
                      <View>
                        <Text style={{ fontSize: 7 * s, color: '#94a3b8', fontWeight: '900', textTransform: 'uppercase' }}>System ID</Text>
                    ...
                    {template.show_qr && <View style={{ padding: 6 * s, backgroundColor: 'white', borderRadius: 12 * s, borderWidth: 1, borderColor: '#f1f5f9' }}><QRCode value={qrPayload} size={44 * s} color={primary} /></View>}
                  </View>
                </View>
              </View>
            </View>
          ...
          return (
            <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 40 * s, overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9' }}>
              ...
              <View style={{ zIndex: 10, flex: 1, alignItems: 'center', paddingTop: 48 * s }}>
                ...
                <Text style={{ fontSize: (template.name_font_size || 24) * s, fontWeight: '900', color: 'white', textAlign: 'center' }}>{displayName}</Text>
                <View style={{ backgroundColor: 'rgba(255,255,255,0.1)', paddingHorizontal: 16 * s, paddingVertical: 6 * s, borderRadius: 20 * s, marginTop: 12 * s, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }}>
                  <Text style={{ fontSize: (template.role_font_size || 10) * s, fontWeight: '900', color: 'white', textTransform: 'uppercase', letterSpacing: 2 * s }}>{displayRole}</Text>
                </View>
                <View style={{ marginTop: 'auto', width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', paddingBottom: 40 * s, paddingHorizontal: 32 * s }}>
                  ...
                  {template.show_id_number && (
                    <View style={{ alignItems: 'flex-end' }}>
                      ...
                      <Text style={{ fontSize: (template.id_number_font_size || 16) * s, fontWeight: '900', color: 'white', letterSpacing: 4 * s }}>{displayID}</Text>
                    </View>
                  )}
                </View>
              </View>
            ...
            <View style={{ flex: 1, borderRadius: 32 * s, overflow: 'hidden', flexDirection: 'row', borderWidth: 1, borderColor: '#f1f5f9' }}>
              ...
              <View style={{ flex: 1, padding: 24 * s, flexDirection: 'row', alignItems: 'center', gap: 24 * s, zIndex: 10 }}>
                ...
                <View style={{ flex: 1, height: '100%', justifyContent: 'center' }}>
                  <Text style={{ fontSize: (template.name_font_size || 24) * s, fontWeight: '900', color: 'white' }}>{displayName}</Text>
                  <View style={{ alignSelf: 'flex-start', backgroundColor: 'rgba(255,255,255,0.1)', paddingHorizontal: 12 * s, paddingVertical: 4 * s, borderRadius: 20 * s, marginTop: 8 * s, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }}>
                    <Text style={{ fontSize: (template.role_font_size || 10) * s, fontWeight: '900', color: 'white', textTransform: 'uppercase' }}>{displayRole}</Text>
                  </View>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto' }}>
                    {template.show_id_number && (
                      <View>
                        <Text style={{ fontSize: 7 * s, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase' }}>System ID</Text>
                        <Text style={{ fontSize: (template.id_number_font_size || 9) * s, fontWeight: '900', color: 'white', letterSpacing: 3 * s }}>{displayID}</Text>
                      </View>
                    )}
          ...
          return (
            <View style={{ flex: 1, backgroundColor: 'white', borderWidth: 2, borderColor: primary, borderRadius: 20 * s, overflow: 'hidden' }}>
              ...
              <View style={{ flex: 1, alignItems: 'center', paddingHorizontal: 32 * s, paddingTop: 24 * s }}>
                ...
                <Text style={{ fontSize: (template.name_font_size || 18) * s, fontWeight: '900', color: primary, textAlign: 'center' }}>{displayName}</Text>
                <Text style={{ fontSize: (template.role_font_size || 10) * s, fontWeight: '700', color: secondary, marginTop: 4 * s }}>{displayRole}</Text>
                ...
                {template.show_id_number && (
                  <View style={{ alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: (template.id_number_font_size || 14) * s, fontWeight: '900', color: primary, letterSpacing: 4 * s }}>{displayID}</Text>
                  </View>
                )}
            ...
            <View style={{ flex: 1, backgroundColor: 'white', borderWidth: 2, borderColor: primary, borderRadius: 20 * s, overflow: 'hidden' }}>
              ...
              <View style={{ flex: 1, flexDirection: 'row', padding: 24 * s, gap: 24 * s }}>
                ...
                <View style={{ flex: 1, height: '100%' }}>
                  <Text style={{ fontSize: (template.name_font_size || 20) * s, fontWeight: '900', color: primary }}>{displayName}</Text>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    {template.show_id_number && (
                      <View>
                        ...
                        <Text style={{ fontSize: (template.id_number_font_size || 10) * s, fontWeight: '900', color: primary, letterSpacing: 3 * s }}>{displayID}</Text>
                      </View>
                    )}
          ...
          return (
            <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 28 * s, overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9' }}>
              ...
              <View style={{ flex: 1, padding: 32 * s, alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: (template.name_font_size || 22) * s, fontWeight: '900', color: '#1e293b' }}>{displayName}</Text>
                <View style={{ width: 32 * s, height: 2 * s, backgroundColor: secondary, marginVertical: 12 * s }} />
                <Text style={{ fontSize: (template.role_font_size || 10) * s, color: secondary, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2 * s }}>{displayRole}</Text>
                <View style={{ marginTop: 'auto', width: '100%', borderTopWidth: 1, borderColor: '#f1f5f9', paddingTop: 16 * s, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                   {template.show_id_number && <Text style={{ fontSize: (template.id_number_font_size || 9) * s, fontWeight: '900', color: '#cbd5e1', letterSpacing: 3 * s }}>{displayID}</Text>}
            ...
            <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 28 * s, overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9', flexDirection: 'row' }}>
              ...
              <View style={{ flex: 1, padding: 32 * s }}>
                ...
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 32 * s }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: (template.name_font_size || 22) * s, fontWeight: '900', color: '#1e293b' }}>{displayName}</Text>
                    <Text style={{ fontSize: (template.role_font_size || 10) * s, color: secondary, fontWeight: '900', textTransform: 'uppercase', marginTop: 4 * s }}>{displayRole}</Text>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto' }}>
                      {template.show_id_number && <Text style={{ fontSize: (template.id_number_font_size || 9) * s, fontWeight: '900', color: '#cbd5e1', letterSpacing: 3 * s }}>{displayID}</Text>}
            ...
            <View style={{ flex: 1, padding: 32 * s, alignItems: isPortrait ? 'center' : 'flex-start', justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: isPortrait ? 'column' : 'row', alignItems: 'center', gap: 32 * s }}>
                ...
                <View style={{ alignItems: isPortrait ? 'center' : 'flex-start' }}>
                  <Text style={{ fontSize: (template.name_font_size || 28) * s, fontWeight: '900', color: 'white', textAlign: isPortrait ? 'center' : 'left' }}>{displayName}</Text>
                  <View style={{ backgroundColor: secondary, paddingHorizontal: 16 * s, paddingVertical: 6 * s, borderRadius: 20 * s, marginTop: 8 * s }}>
                    <Text style={{ fontSize: (template.role_font_size || 9) * s, fontWeight: '900', color: 'white', textTransform: 'uppercase' }}>{displayRole}</Text>
                  </View>
                </View>
              </View>
          ...
          <View style={{ flex: 1, backgroundColor: 'white', borderWidth: 4, borderColor: primary, borderRadius: 16 * s, overflow: 'hidden' }}>
             ...
             <View style={{ flex: 1, flexDirection: isPortrait ? 'column' : 'row', padding: 24 * s, gap: 24 * s, alignItems: 'center' }}>
                ...
                <View style={{ flex: 1, width: '100%' }}>
                  <Text style={{ fontSize: (template.name_font_size || 22) * s, fontWeight: '900', color: primary, textTransform: 'uppercase' }}>{displayName}</Text>
                  <Text style={{ fontSize: (template.role_font_size || 10) * s, fontWeight: '700', color: secondary }}>{displayRole}</Text>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    {template.show_id_number && (
                      <View>
                        ...
                        <Text style={{ fontSize: 11 * s, fontWeight: '900', color: primary }}>{displayID}</Text>
                      </View>
                    )}
                    {template.show_qr && <QRCode value={qrPayload} size={44 * s} color={primary} />}
                  </View>
          ...
          <View style={{ flex: 1, backgroundColor: '#0f172a', borderRadius: 32 * s, overflow: 'hidden', position: 'relative' }}>
            ...
            <View style={{ flex: 1, padding: 32 * s, alignItems: 'center', justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: isPortrait ? 'column' : 'row', alignItems: 'center', gap: 32 * s, marginVertical: 32 * s }}>
                ...
                <View style={{ alignItems: isPortrait ? 'center' : 'flex-start' }}>
                  <Text style={{ fontSize: (template.name_font_size || 24) * s, fontWeight: '900', color: 'white' }}>{displayName}</Text>
                  <Text style={{ fontSize: (template.role_font_size || 10) * s, fontWeight: '700', color: primary, textTransform: 'uppercase', marginTop: 4 * s }}>{displayRole}</Text>
                </View>
              </View>
              <View style={{ marginTop: 'auto', width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                {template.show_id_number && (
                  <View>
                    ...
                    <Text style={{ fontSize: 12 * s, fontWeight: '900', color: 'white' }}>{displayID}</Text>
                  </View>
                )}
  ...
  const renderBack = () => {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: isBackDark ? primary : 'white', borderRadius: 24 * s, overflow: 'hidden', borderWidth: 1, borderColor: '#f1f5f9' }}>
        ...
        <View style={{ flex: 1, padding: 32 * s, alignItems: 'center' }}>
          {template.custom_back_bg_url && <Image source={{ uri: resolveImageUrl(template.custom_back_bg_url), headers: { 'Bypass-Tunnel-Reminder': 'true' } }} style={{ position: 'absolute', inset: 0, opacity: 0.4 }} contentFit="cover" />}
          ...
          <View style={{ marginTop: 'auto', width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <View style={{ alignItems: 'center', flex: 1 }}>
              ...
              <View style={{ width: 80 * s, height: 1, backgroundColor: mainColor, opacity: 0.2, marginVertical: 4 * s }} />
              <Text style={{ fontSize: 6 * s, fontWeight: '900', color: subColor, textTransform: 'uppercase' }}>Cardholder</Text>
            </View>
            <View style={{ alignItems: 'center', flex: 1 }}>
              <View style={{ height: 40 * s, marginBottom: -10 * s, zIndex: 10 }}>
              ...
              <View style={{ width: 80 * s, height: 1, backgroundColor: mainColor, opacity: 0.2, marginVertical: 4 * s }} />
              <Text style={{ fontSize: 6 * s, fontWeight: '900', color: subColor, textTransform: 'uppercase' }}>{template.signature_label || "Authorized"}</Text>
            </View>
          </View>
          {template.show_barcode && (
            <View style={{ marginTop: 32 * s, alignItems: 'center', width: '100%' }}>
...
def main():
    ...
    if start_idx != -1 and end_idx != -1:
        ...
        with open(FILE_PATH, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Master reconstruction completed successfully")
    else:
